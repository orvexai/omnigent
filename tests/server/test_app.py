...
async def test_readiness_returns_ok_when_database_is_reachable(
    ...
    """GET /ready returns 200 when its database query succeeds."""
...
async def test_readiness_fails_when_database_is_unreachable_but_health_stays_live(
    ...
    """Readiness withholds traffic for DB failure without changing /health."""

    def fail_database_check(_store: SqlAlchemyConversationStore, *, timeout_ms: int = 500) -> None:
    ...
    assert readiness.status_code == 503, (
        f"/health stayed {health.status_code} with body {health.json()!r}"
    )
...
@pytest.mark.asyncio
async def test_readiness_caches_database_result_for_one_second(
    client: httpx.AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Repeated probes within the cache window issue one database check."""
    checks = 0

    def count_database_check(
        _store: SqlAlchemyConversationStore, *, timeout_ms: int = 500
    ) -> None:
        nonlocal checks
        checks += 1
