...
class SysSessionSendTool(Tool):
    ...
    def description(cls) -> str:
        ...
        return (
            ...
            "the sending session, so the receiver knows it is talking to an "
            "agent; its reply returns through sys_read_inbox, not as a direct "
...
def _build_sys_session_send_schema(
    ...
    args_description = (
        (
            "The user-input message is delivered wrapped in a provenance "
            "envelope naming the sending session, so the receiver knows it "
            "is talking to an agent; its final turn reply returns through "
            "sys_read_inbox, not as a direct return value. Pass a "
...
class SysSessionGetInfoTool(Tool):
    ...
    def description(cls) -> str:
        ...
        return (
            ...
            "(a child with no placement of its own reports its nearest "
            "placed ancestor's; placement_inherited is true when the "
            "values came from an ancestor, false when they are the "
            "session's own, and null when an ancestor lookup failed so "
            ...
            "last_task_error: null when the session has not failed, or "
            "{code, message} with optional title, cause, and remediation "
            "when the last launch or turn failed. A sys_session_create "
...
class SysSessionCreateTool(Tool):
    ...
    def description(cls) -> str:
        ...
        return (
            ...
            "Returns {conversation_id, agent_id, title, status} as an "
            "acceptance receipt, not confirmation that the session has "
            "started. Poll sys_session_get_info and inspect "
            "last_task_error to confirm launch; a non-null value is "
            "terminal for that launch. The session runs asynchronously — "
            "monitor it with sys_session_get_history / sys_session_get_info "
            "or drive it with sys_session_send. Optionally set its reasoning "
            "effort."
        )
