    ...
    def _start_locked(self) -> None:
        ...
        if IS_WINDOWS:
            if self._tmpdir is None:
                # Only the ``sandbox.active`` branch above creates the private
                # tmpdir, but this Windows config-file handoff needs one
                # regardless — POSIX gets by with an inherited pipe and no
                # directory at all. Without this the helper could never start
                # under an inactive sandbox: the assert that used to stand here
                # fired on every request (e.g. the web file browser's
                # ``list_environment_root``). ``_stop_locked`` already cleans it
                # up, and no sandbox wiring is needed since none is active.
                self._tmpdir = create_private_tmpdir()
