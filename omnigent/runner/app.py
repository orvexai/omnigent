...
def create_runner_app(
    *,
    ...
    def _create_workspace_registry(watch_path: Path) -> FilesystemRegistry:
        return create_filesystem_registry(watch_path=watch_path)

    async def _create_workspace_registry_off_loop(watch_path: Path) -> FilesystemRegistry:
        """Create a registry without running Git detection on the event loop."""
        return await asyncio.to_thread(_create_workspace_registry, watch_path)
    ...
    async def _resolve_session_fs_registry(
        ...
        registry = await _create_workspace_registry_off_loop(session_ws_path)
        registry.start()
    ...
    async def read_environment_file_diff(
        ...
        try:
            # ``get_changed_file`` shells out to Git synchronously; keep it off
            # the event loop like ``list_filesystem_changes``.
            record = (
                await asyncio.to_thread(
        ...
        before: str | None = (
            await asyncio.to_thread(session_registry.get_baseline, relative_path)
            if session_registry is not None
            else None
        )
